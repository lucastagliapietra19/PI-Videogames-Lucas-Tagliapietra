...
export const getVideogames = (name) => {
  return async function (dispatch) {
    ...
    if (name) {
      try {
        const { data } = await apiRequest.get(`${RUTA_VIDEOGAMES}?name=${name}`)
        console.log(data);
      ...
      try {
        const { data } = await apiRequest.get(`${RUTA_VIDEOGAMES}`)
        const { ok, arrVideogames } = data
        if (ok) {
          dispatch({
            type: GET_VIDEOGAMES,
...
export const getVideogameById = (id) => {
  return async function (dispatch) {
    ...
    try {
      const { data } = await apiRequest.get(`${RUTA_VIDEOGAMES}/${id}`)
...
export const addVideogame = (newVideogame) => {
  return async function (dispatch) {
    try {
      const { data } = await apiRequest.post(`${RUTA_VIDEOGAMES}`, newVideogame)
      const { ok, createdVideogame, message } = data
      if (ok) {
        dispatch({
          type: ADD_VIDEOGAME,
...
export const getGenres = () => {
  return async function (dispatch) {
    try {
      const { data } = await apiRequest.get(`${RUTA_GENRES}`)
